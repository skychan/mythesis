...
\begin{asparaenum}
\renewcommand{\labelenumi}{\bf Step\theenumi~}
\item 初始化。$J = N$, $t_l = 0, \overline{S_l} = \varnothing, a_l=0 (l = 1,2,...,m)$，根据\eqref{equ:processing}和\refa{alg:processtime}计算各订单处理时间$p'_j = f(j, n_j)$，进一步得到整合订单处理时间$p_j = p'_j + s_j(j = 1,2,...,n)$，置系统时间$t = 0$；
\item 若存在$a_l = 0$，记$l^* = \displaystyle\min_{a_l = 0}\{l\}$，执行\Step{3}，否则执行\Step{4}；
\item 根据\eqref{equ:orderindexexample}，选取预备调度订单$j^*$，使得$I_{j^*}(t) = \displaystyle\max_{j\in J}\{I_j(t)\}$，将订单$j^*$安排入产线$l^*$进行处理，记入调度$S_{l^*}$，$\overline{S_{l^*}}=\overline{S_{l^*}}\bigcup \{j^*\}$, $J = J -\{j^*\}$，更新产线预计空闲时刻$t_{l^*} = t + p_{j^*}$，修改产线状态$a_{l^*} = 1$。若$J = \varnothing$，所有订单调度完毕，终止算法，否则执行\Step{2}
\item 记$lt$使得 $t_{lt} = \displaystyle\min_{1\le l\le m}\{t_l\}$，修改产线状态$a_{lt} = 0$，并更新系统时间$t = t_{lt}$，执行\Step{2}
\end{asparaenum}
\end{algori}


显而易见的是，由该算法得到的调度，对于每条产线来说，其上的订单都按照ATC规则排序。
